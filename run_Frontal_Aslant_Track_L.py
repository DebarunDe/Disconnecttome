	...
	if WMH.endswith(".nii.gz"):
		...
		for FIB in os.listdir(directory):
			if FIB.endswith(".src.gz.gqi.1.7.fib.gz"):
				...
				o = ""+o_FIB+".Frontal_Aslant_Track_L_"+o_WMH+".tt.gz"
				output = os.path.join(directory,o)
				cmd = "dsi_studio --action=trk --track_id=Frontal_Aslant_Track_L --source="+f2+" --seed_count=100000 --tip_iteration=0 --roa="+f1+" --output="+output+" --export=stat"
				os.system(cmd)
