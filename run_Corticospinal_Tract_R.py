	...
	if WMH.endswith(".nii.gz"):
		...
		for FIB in os.listdir(directory):
			if FIB.endswith(".src.gz.gqi.1.7.fib.gz"):
				...
				o = ""+o_FIB+".Corticospinal_Tract_R_"+o_WMH+".tt.gz"
				output = os.path.join(directory,o)
				cmd = "dsi_studio --action=trk --track_id=Corticospinal_Tract_L --source="+f2+" --seed_count=100000 --tip_iteration=0 --roa="+f1+" --output="+output+" --export=stat"
				os.system(cmd)
